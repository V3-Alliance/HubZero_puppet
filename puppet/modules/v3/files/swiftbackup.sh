...
# keystone client writes warnings to syserr: which automysql backup then assumes to mean an error occurred.
# hence we trap the error stream to a variable, then, if duplicity fails, echo it back into the syserr stream.
